...
供货：
    主键：（供应商名称、医院名称、供货内容）；
    外键：供应商名称、医院名称；
    供应商名称是供应商关系模式的主键，医院名称是医院关系模式的主键；
(b)接种日期；
接种：
    主键：（被接种者身份证号、接种日期）;
    外键：被接种者身份证号，医院名称，供应商名称；
被接种者身份证号是被接种者关系模式中的主键，医院名称是医院关系模式中的主键供应商名称是供应商关系模式中的主键；
```

```
问题 3
被检测者 * --- 检测 --- * 医院；
被检测者（姓名，被检测者身份证号，住址、电话）；
...
AddressBook：
    属性：部门，员工地址列表；
    方法：创建地址簿，打开地址簿，修改地址簿，保存地址簿，打印地址记录，按姓氏排序地址簿记录，按邮政编码排序地址簿记录；
PersonAddress：
    属性：姓名，住址，城市，省份，邮政编码，电话；
```

```
问题 3
《include》：包含关系，被包含用例在包含用例中会用得到的用例，是包含用例主功能必不可少的；
《extend》：扩展关系，扩展用例对被扩展关系的使用，主用例对被扩展用例不是必须的，只是可能会被用到的功能；
```
